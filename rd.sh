while getopts a:q:r:y:l flag
	...
	function yts() {
	        local response=$(curl -s -X GET "$ytsurl?language="en"&quality="1080p"&page=$1&limit=50" | jq -r --arg quality $quality --arg lang $language '.data.movies[] | select(.language==$lang) | .torrents[] | select(.quality==$quality) | .hash')
	...
	if [ -f "$ytsfile" ]; then
	  ...
	  mapfile -t ytshashes < $ytsfile
	        ...
	        ytsfile="yts.txt"
	        # Init ytshashes
	        moviescount=$(curl -s -X GET "$ytsurl?quality=$quality" | jq .data.movie_count)
	        pages=$(( (moviescount + 50 - 1) / 50 ))
	        page=0
	        curl -s -X GET "$ytsurl?language="en"&quality="1080p"&page=$1&limit=50" | jq -r --arg quality $quality --arg lang $language '.data.movies[] | select(.language==$lang) | .torrents[] | select(.quality==$quality) | .hash' | sort | uniq -u > yts.txt
	        # YTS Loop
	        while [ $page -lt $pages ]
	        do
	                yts $page $pages &
